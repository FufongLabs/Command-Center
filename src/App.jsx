             ...
             <div key={field.key}>
                ...
                {field.key === 'tag' && (
                    <div className="mt-3 flex flex-wrap gap-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                        ...
                        {PRESET_TAGS.map(tag => (<button key={tag} onClick={() => setFormData({...formData, tag: tag})} className={`text-[10px] px-2.5 py-1.5 rounded-full border font-medium transition-all active:scale-95 ${formData.tag === tag ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white text-slate-500 border-slate-200 hover:border-blue-300'}`}>{tag}</button>))}
                    </div>
                )}
             </div>
           ))}
...
const LoginScreen = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
         // Set default role as Member. Manually change to Admin in Firestore to see Admin panel.
...
export default function TeamTaweeApp() {
  ...
  const addNewTask = (key) => openFormModal("เพิ่มงานใหม่", [{key:'title', label:'ชื่องาน'}, {key:'tag', label:'Tag'}, {key:'role', label:'ผู้รับผิดชอบ'}, {key:'deadline', label:'กำหนดส่ง', type:'date'}], async(d)=>{ await addDoc(collection(db,"tasks"), {...d, status:"To Do", link:"", columnKey:key, createdBy:currentUser.displayName, createdAt:new Date().toISOString()}); logActivity("Add Task", d.title); });
  ...
  const updateDist = (id, c) => updateDoc(doc(db,"channels",id), {count:Math.max(0,c)});
  const deleteChannel = (id) => { if(confirm("ลบ?")) { deleteDoc(doc(db,"channels",id)); logActivity("Delete Channel", id); }};
  const toggleMediaActive = (c) => updateDoc(doc(db,"media",c.id), {active:!c.active});
  const deleteMedia = (id) => { if(confirm("ลบ?")) { deleteDoc(doc(db,"media",id)); logActivity("Delete Media", id); }};
  ...
  const deleteLink = (id) => { if(confirm("ลบ?")) deleteDoc(doc(db,"published_links",id)); };
  ...
  const addPlan = () => openFormModal("สร้างแผนใหม่", [{key:'title', label:'ชื่อแผน'}], async(d)=> addDoc(collection(db,"plans"), {...d, progress:0, items:[]}));
          ...
          <div className="space-y-6 animate-fadeIn">
             <PageHeader title="ภาพรวมสถานการณ์" subtitle="Overview & Statistics" action={
                 <div className="flex gap-2">
                    <button onClick={() => setIsSearchOpen(true)} className="p-2 bg-white border rounded-lg text-slate-500 hover:text-blue-600 shadow-sm"><Search className="w-5 h-5" /></button>
                    <button onClick={() => addNewTask('solver')} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-blue-700 shadow-lg flex items-center gap-2"><Plus className="w-4 h-4" /> เพิ่มงาน</button>
                    <button onClick={createUrgentCase} className="bg-red-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-red-700 shadow-lg flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> เคสด่วน</button>
                 </div>
             } />
             {/* 3 Columns Layout */}
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col">
                    ...
                    <div className="grid grid-cols-2 gap-3 flex-1 content-start">
                        {channels.slice(0,4).map(item => (
                            <div key={item.id} className="bg-slate-50 p-2 rounded border text-center group relative">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col">
                    <div className="flex justify-between items-center mb-4"><p className="text-slate-500 text-xs font-bold uppercase">Master Plan</p><button onClick={()=>navigateTo('masterplan')} className="text-xs text-blue-600 hover:underline">ดูทั้งหมด →</button></div>
             ...
             <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {['solver', 'principles', 'defender', 'expert'].map(k=><div key={k} className="bg-slate-50 p-3 rounded border h-32 relative flex flex-col justify-center items-center cursor-pointer hover:border-blue-300" onClick={()=>navigateTo('strategy')}><span className="text-[10px] font-bold uppercase text-slate-400 mb-1">{k}</span><span className="text-3xl font-black text-slate-700">{groupedTasks[k]?.length||0}</span></div>)}
                 </div>
             </div>
             {/* News Links */}
             <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                 <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition" onClick={()=>setIsDistOpen(!isDistOpen)}>
                     <div className="flex items-center gap-2"><LinkIcon className="w-4 h-4 text-slate-500"/><h3 className="font-bold text-sm text-slate-700">ลิงก์ข่าวที่ลงแล้ว</h3></div>{isDistOpen?<ChevronUp className="w-4 h-4"/>:<ChevronDown className="w-4 h-4"/>}
                 </div>
                 {isDistOpen && <div className="p-4 max-h-60 overflow-y-auto custom-scrollbar bg-white"><button onClick={addPublishedLink} className="w-full text-xs bg-blue-50 text-blue-600 py-2 rounded border font-bold mb-3">+ เพิ่มลิงก์</button><div className="space-y-2">{publishedLinks.map(l=><div key={l.id} className="flex justify-between p-2 border rounded hover:bg-slate-50 group"><a href={l.url} target="_blank" className="text-xs text-blue-700 truncate w-full font-medium block">{l.title}</a><button onClick={()=>deleteLink(l.id)}><Trash2 className="w-3 h-3 text-slate-300 hover:text-red-500"/></button></div>)}</div></div>}
             </div>
          </div>
        );
        ...
        <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-2/3 space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mt-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">{media.filter(c=>c.active).map((c,i)=><div key={i} className="p-4 border rounded-xl"><p className="font-bold text-sm">{c.name}</p><div className="text-xs text-slate-500 flex flex-col gap-1 mt-1"><span>📞 {c.phone}</span><span>LINE: {c.line}</span></div></div>)}</div>
                </div>
            </div>
        </div>
