...
// Form Modal (Fixed: Ensure fields prop is used correctly)
        ...
        return (
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm mt-2">
                 <div className="flex justify-between items-center mb-4"><p className="text-slate-500 text-xs font-bold uppercase">Master Plan Progress</p><button onClick={() => navigateTo('masterplan')} className="text-xs text-blue-600 font-bold hover:underline">ดูรายละเอียด →</button></div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {media.filter(c => c.active).map((c,i) => (
                                    <div key={i} className="p-4 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-white hover:shadow-md transition group">
                                        ...
                                        {/* ADDED: Edit Button for Quick Contacts */}
                                        <button onClick={(e) => {e.stopPropagation(); editMedia(c);}} className="absolute top-2 right-2 p-1 text-slate-300 hover:text-blue-600 transition"><Edit2 className="w-3.5 h-3.5" /></button>
                                    </div>
                                ))}
                            </div>
