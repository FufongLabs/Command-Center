...
const formatForInput = (timestamp) => {
 ...
 // ฟังก์ชันเพิ่มข่าว (ฉบับบังคับตรวจทาน: Fetch -> Review -> Save)
  const addPublishedLink = (prefillData = null) => {
    // เช็คว่ามีข้อมูลส่งมาหรือยัง (ถ้ามี Title แปลว่าผ่านการดึงมาแล้ว)
    const isReviewMode = prefillData && prefillData.title;
    ...
    openFormModal(isReviewMode ? "ตรวจสอบความถูกต้อง (Review)" : "เพิ่มลิงก์ข่าว", [
      ...
      // --- PHASE 1: ถ้าเป็นรอบแรก (ยังไม่มีชื่อเรื่อง) ให้วิ่งไปดึงข้อมูลก่อน ---
      if (d.url && !d.title) {
          try {
              const meta = await fetchLinkMetadata(d.url);
              
              // เตรียมข้อมูลที่จะใส่กลับเข้าไปในฟอร์ม
              let nextData = {
                  url: d.url,
                  title: meta?.title || "", // ถ้าไม่เจอให้ว่างไว้ ให้ User พิมพ์เอง
                  imageUrl: meta?.image || "",
                  platform: d.platform,
                  customDate: formatForInput(new Date()) // Default วันนี้
              };

              // ถ้าดึงวันที่ได้ ให้ใช้ตัวนั้น
              if (meta && meta.date) {
              ...
              // 🟢 ยังไม่บันทึก! แต่เปิดฟอร์มซ้ำพร้อมข้อมูลที่ดึงมาให้คุณตรวจ
              setTimeout(() => {
                  addPublishedLink(nextData);
              }, 100);
              return; // *** จบการทำงานรอบแรก (Stop) ***
          } catch (e) { 
              console.log(e);
              // ถ้า Error ให้เปิดฟอร์มเปล่าๆ กลับมาให้กรอกมือ
              setTimeout(() => addPublishedLink({ ...d, title: " " }), 100); 
              return;
          }
      }

      // --- PHASE 2: บันทึกจริง (เมื่อมีข้อมูลครบและ User กดยืนยัน) ---
    ...
    }, isReviewMode ? "ยืนยันการบันทึก" : "ดึงข้อมูล"); // เปลี่ยนชื่อปุ่มให้ชัดเจน
  };
