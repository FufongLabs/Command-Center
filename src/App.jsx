...
// --- GLOBAL CONSTANTS (ประกาศไว้บนสุด เพื่อกัน Error) ---
const PRESET_TAGS = [
  ...
  const createUrgentCase = async () => {
    openFormModal("เปิดเคสด่วน (New Urgent Case)", [
        ...
        await addDoc(collection(db, "tasks"), { 
            ...
            sop: DEFAULT_SOP, // Now Correctly Defined
            createdBy: currentUser.displayName, createdAt: new Date().toISOString()
        });
                  ...
                  <div className="bg-slate-50 rounded-lg p-4">
                    ...
                    <ul className="space-y-2">
                      {sortedItems.map((item, idx) => {
                         ...
                         return (
                          <li key={idx} className={`flex items-center justify-between gap-2 text-sm p-1.5 rounded hover:bg-white transition ${item.completed ? 'opacity-40 order-last' : ''}`}>
                            <div className="flex items-center gap-2 cursor-pointer flex-1" onClick={() => togglePlanItem(plan.id, originalIndex, plan.items)}>
                             ...
                             {/* FIXED: Separate buttons area with z-index to ensure clickable */}
