                    ...
                    <ul className="space-y-2">
                      {sortedItems.map((item, idx) => (
                          <li key={idx} className={`flex items-center justify-between gap-2 text-sm p-1.5 rounded hover:bg-white transition ${item.completed ? 'opacity-40 order-last' : ''}`}>
                            <div className="flex items-center gap-2 cursor-pointer flex-1" onClick={() => togglePlanItem(plan.id, item.originalIndex, plan.items)}>
                            ...
                            <div className="flex gap-1 relative z-10">
                                <button onClick={(e) => { e.stopPropagation(); editPlanItem(plan.id, item.originalIndex, plan.items); }} className="text-slate-400 hover:text-blue-600 p-1.5 rounded hover:bg-blue-50"><Edit2 className="w-3.5 h-3.5" /></button>
                                <button onClick={(e) => { e.stopPropagation(); removePlanItem(plan.id, item.originalIndex, plan.items); }} className="text-slate-400 hover:text-red-500 p-1.5 rounded hover:bg-red-50"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                          </li>
                      ))}
